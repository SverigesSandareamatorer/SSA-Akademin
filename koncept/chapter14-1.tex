...
\begin{figure}
  \begin{center}
    \begin{circuitikz}[american voltages]
      ...
      \draw (0,1) to (7,1);
      % Source ground
      \draw (0,4) to [american voltage source, l^=$U_{cm}$] (0,1);
      \draw (0,4) to (2,4);
      % Source diff
      \draw (2,6) to [american voltage source, l^=$U_{dm}/2$] (2,4)
      to [american voltage source, l^=$U_{dm}/2$] (2,2);
      % Wires and load
      \draw (2,6) to [R, l^=$Z_{signal+}$] (5,6)
      to [R, l^=$Z_{dm}/2$, v_=$U_{dm+}$] (5,4)
      to [R, l^=$Z_{dm}/2$, v_=$U_{dm-}$] (5,2);
      \draw (2,2) to [R, l^=$Z_{signal-}$] (5,2);
      \draw (2,4) to [R, l^=$Z_{GND}$] (5,4);
      % Destination isolation
      \draw (5,4) to (7,4);
      % Destination ground
      \draw (7,4) to [R, l^=$Z_{cm}$, v_=$U_{cm}$] (7,1);
    \end{circuitikz}
  \end{center}
  \caption{Sammankopplat system med utjämningsledare och diffrentiell signal}
  \label{fig:kap4-7}
