...
Skyddsjorden skall ha väldigt lite ström på sig, och därmed även ha väldigt
...
Detta förekommer t.ex. i en serie av elurtag matade från samma säkring
eller flera elurtag i en skarvdosa.
...
Den diffrentiella spänningen \(U_d\), dvs. den drivande spänningen mellan
\(V_{ut+}\) och  \(V_{ut-}\) är fördelad på två spänningskällor som levererar
halva spänningen var.

\(V_{ut+} = U_g + \frac{U_d}{2}\)

\(V_{ut-} = U_g - \frac{U_d}{2}\)

Omvänt kan man formulera uttrycken för gemensam spänningsen \(U_g\) samt
den diffrentiella spänningen \(U_d\) som \(V_{ut+}\) och \(V_{ut-}\):

\(U_g = \frac{V_{ut+}+V_{ut-}}{2}\)

\(U_d = {V_{ut+}-V_{ut-}}\)

På motsvarande sätt på ingången kan man skriva uttrycken för den gemensamma
mottagna spänningen \(U_{g,in}\) och den mottagna diffrentella spänningen
\(U_{d,in}\) baserat på inspänningarna \(V_{in+}\) och \(V_{in-}\), man får då

\(V_{g,in} = \frac{V_{in+} + V_{in-}}{2}\)
...
Däremot kommer den gemensamma spänningen att inte överföras.
...
kabeln flera varv om den, eller bara lindar den runt luft.
...
som kan lågresitivt hindra koppling.
...
Principen återkommer oavsett spänning och frekvens, det är en imperfektion
...
till antennen.
Strömbalunen motverkar ju den gemensamma strömmen utan att nämnvärt påverka
