...
<<<<<<< HEAD
Före \emph{WARC 79} var symbolen för all pulssändning P, därefter används P 
...
som t.ex. att en fil är \SI{2}{kB} eller en bild är \SI{1,25}{MB}. Då en byte 
innehåller 8 bitar, så motsvarar det \SI{16}{kbit} respektive \SI{10}{Mbit}.
...
om i termer av \emph{bit rate} som \SI{10}{Mbit/s} (ofta skrivet \emph{bps --- 
bits per second}), dvs. man klarar av att överföra upp till 10 miljoner bitar 
per sekund.
...
<<<<<<< HEAD
sänder samma tecken två gånger. Liknande används i Bluetooth där 
meddelandet sänds tre gånger, varvid man kan göra majoritetsröstning.
=======
