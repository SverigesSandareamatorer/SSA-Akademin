...
det vill säga antingen bärvåg med någon varaktighet eller ingen bärvåg alls.
Kombinationer av bärvågselement med olika längd motsvarar skrivtecken.

För att återge tal, musik etcetera behövs en noggrannare tillståndsstyrning av
bärvågen.
...
att utsända information över radio med mera.
...
Ett vanligt sätt att överföra information över radio är med telefoni, det vill säga
tal.
...
\item Anslut en mikrofon till ett oscilloskop och studera spänningsförloppen
  för olika slags ljud, toner, tal och så vidare som funktion av tiden.
...
bärvågen, det vill säga en sjättedel av den totalt utsända effekten.
...
frekvenserna 5~Hz, 15~Hz, 25~Hz, 35~Hz och så vidare.

Det innebär att det uppstår sidofrekvenser över och under bärvågens frekvens och
med ett avstånd till bärvågen av 5~Hz, 15~Hz, 25~Hz, 35~Hz och så vidare.
...
och skillnadsfrekvenser blir kvar, det vill säga det övre och nedre sidbandet.
...
Bärvågen kommer alltså att pendla omkring ett medelvärde, det vill säga vara
frekvensmodulerad.
...
sekund, vid 3~kHz varierar den 3000 gånger per sekund.
...
\(f_{LFmax} = 3~kHz\), det vill säga \(m = 1\).
...
\item Fasmodulation (PM) alstras vanligen genom att efter sändaroscillatorn
  ...
  oscillatorn, det vill säga utan att påverka oscillatorfrekvensen.
...
skiljer sig från CW-modulationen genom att den ändrar frekvensen, det vill säga är en
...
att bli inverterat, det vill säga 0 blir 1 och 1 blir 0. BPSK behöver därför också
...
större frekvensband. I mobiltelefonisystem använder man bland annat 
64QAM och 256QAM.
...
som till exempel att en fil är 2~kB eller en bild är 1,25~MB. Då en byte innehåller
8 bitar motsvarar det 16~kb respektive 10~Mb.
...
second}), det vill säga man klarar av att överföra upp till 10 miljoner bitar per sekund.
...
nära en annan signal man kan ligga i spektrat utan att störa varandra, det vill säga
...
Av olika orsaker kommer en eller flera bitar ofta att bli fel. Vi kallar varje sådant 
fel för att \emph{bitfel} (eng. \emph{bit error}). Störningar kan göra att vi tolkar 
en symbol fel, vilket kan resultera i en eller flera felaktiga bitar.
...
När gränsen är 1 sigma, det vill säga 1 gånger RMS-värdet för brusamplituden, från
medelvärdet så är det 67~\% sannolikhet att värdet ligger inom gränsvärdet, det vill säga en
...
det 14 sigma bort till gränsen, det vill säga brusmängden får max vara 1/14 av
...
används till exempel i seriekommunikation så som RS-232.
...
försvunnit. Ett sådant protokoll är TCP, som ingår i internetsviten av TCP/IP-protokollet.
...
sänds tre gånger, varvid man kan göra majoritetsröstning.
...
De flesta digitala sändningsslagen för kortvåg är smalbandiga och bandbredden kan i vissa
...
uttytt ''RadioTeleTYpe'', där man använde sig av s.k. teleprintrar,
...
Joe Taylor fick sin utbildning i astronomi vid Harvard University var sedan verksam 
inom området astrofysik vid Princeton University, varifrån han pensionerades 2006.
Han tilldelades Nobelpriset i fysik år 1993.
...
meteorreflektioner på 6~m-bandet.
...
av PSK-modulering.
Bandbredden är hälften (1,25~kHz) av en normal SSB-bandbredd (2,4~kHz).
