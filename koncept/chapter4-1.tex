...
olika signaler.
Den första enkla separationen är den hos en \emph{isolator}, det vill säga ett material
...
jordpotential, det vill säga \(U_1 - U_2\) som i sin tur beror på anslutningarnas
...
\emph{floating}), det vill säga utan någon galvanisk koppling till skyddsjord, så kan
...
Den differentiella spänningen \(U_d\), det vill säga den drivande spänningen mellan
...
likströmsbeteende, så ju högre frekvens desto bättre koppling, det vill säga sämre
isolation.
...
transformatorer, induktorer med mera är ideala och ger lika bra koppling till bägge
sidor.
...
(single-ended) det vill säga det är mittledaren i förhållande till skärm/jord som
överför energi.
...
energin där den ska, det vill säga radierat ut i luften på ett korrekt sätt, men även
