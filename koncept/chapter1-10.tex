...
\emph{Digital Signalprocessor (DSP)}. Behandlingen kan även göras av dedikerad
logik, alltså logik avsedd för speciellt ändamål, som inte kan programmeras på
normalt sätt som en processor.
...
\marginnote{HAREC a.\ref{HAREC.a.1.10.1}\label{myHAREC.a.1.10.1}}
\index{sampling}
\index{samplingstakt}
\index{sample rate}
...
Denna sista del i att omvandla de kvantiserade talen till värden
kallas \emph{Pulse Coded Modulation (PCM)}. Omvandlingen kan även ske
olinjärt, alltså med olika avstånd mellan stegen i
kvantiseringstrappan, vilket nyttjats för datakompression\footnote{I
  litteraturen beskrivs detta ibland som dynamikkompression och det är
  det egentligen också. Termen compander myntades någonstans här för
  att beskriva \emph{compressor---expander} men det är i praktiken
  antalet databitar som reduceras från 14 bitar som egentligen behövs
  för att nå de ca 80 dB dynamik man behöver i telefonisystem till de
  8 bitar i PCM-länken som används.}
telefonisystem.
% Förklaring till varför det kallas compander eller dynamikkompressor i
% litteraturen. /SM0UEI

\subsection{Minsta samplingsfrekvensen}
\marginnote{HAREC a.\ref{HAREC.a.1.10.2}\label{myHAREC.a.1.10.2}}
\index{nyquistfrekvens}
\index{Nyquist-Shannons samplingsteorem}

\marginnote{Denna frekvens kallas för nyquistfrekvensen efter Harry Nyquist
  (1889--1976), från Stora Kil i Värmland, efter hans banbrytande arbete på Bell
  laboratories som han publicerade åren 1924 och 1928.
  Den ingår i \emph{Nyquist-Shannons samplingsteorem} (eng.
...
\subsection{Faltning}
\marginnote{HAREC a.\ref{HAREC.a.1.10.3}\label{myHAREC.a.1.10.3}}
\index{faltning}
\index{convolution}
\index{konvolution}
...
\marginnote{HAREC a.\ref{HAREC.a.1.10.4}\label{myHAREC.a.1.10.4}}
\index{vikning}
\index{aliasing}
